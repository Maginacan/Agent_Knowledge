...
| 模式 | 它继续解决什么 | 与工具调度的接缝 |
|------|--------------|---------------|
| **[[lession25-规划执行\|规划执行]]** | 多步任务怎样保持全局目标与局部恢复 | 每个计划步骤内部调用 Dispatcher |
| **[[lession26-提示链\|提示链]]** | 上一步工件怎样不污染下一步 | 工件通过段间闸门后才形成行动意图 |
| **[[lession27-护栏三明治\|护栏三明治]]** | 高风险动作前后分别检查什么 | 准入通过后仍经过 PRE、TOOL、POST |
