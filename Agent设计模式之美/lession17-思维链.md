...
- **不会同时维护很多条候选路径再投票** → 那是 [[lession19-并行探索\|并行探索]]
- **不是持续观察环境、不断重写假设的迭代闭环** → 那是 [[lession20-迭代假设验证\|迭代假设验证]]
...
>
> 它其实和上次提到的 [[lession15-失败日记\|失败日记]] 有点像。二者的方向都是希望让 Agent 的执行更加工程化，而不是简单地换一个能力更强的 Agent，然后让它在没有约束的情况下自由发挥。
>
